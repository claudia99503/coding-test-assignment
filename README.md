...
- **Next.js 14 (App Router)**
  - **선택 이유**: 파일 기반 라우팅으로 페이지 구조를 직관적으로 관리할 수 있어 유지보수가 용이함. 서버/클라이언트 컴포넌트 구분으로 상태 관리가 필요한 컴포넌트에만 클라이언트 번들에 포함시켜 성능 최적화. 동적 라우팅을 쉽게 구현하여 동적 페이지 구현이 간편함. 내장 이미지 최적화 기능을 활용하여 아이콘을 최적화된 형태로 사용

- **TypeScript**
  - **선택 이유**: 타입 정의를 통해 API 응답 데이터 구조를 명시적으로 관리. 프로퍼티 접근 시 오타나 잘못된 타입 사용을 컴파일 단계에서 방지. 프로퍼티 자동완성으로 개발 생산성 향상. 타입 체크를 통해 함수 시그니처 변경 시 영향받는 모든 코드를 컴파일 단계에서 발견 가능

### 스타일링

- **Tailwind CSS**
  - **선택 이유**: 별도 CSS 파일 작성 없이 클래스명으로 스타일링. 브레이크포인트별 스타일 적용으로 반응형 레이아웃 구현이 간편함. 스타일 상수를 추출하여 재사용성 향상. CSS 모듈이나 Styled Components 없이 순수 Tailwind로 구현하여 프로젝트 복잡도 감소

### 주요 라이브러리

- **react-day-picker**
  - **선택 이유**: 날짜 범위 선택 기능을 직접 구현하는 것보다 검증된 라이브러리를 사용하여 개발 시간 단축 및 버그 위험 감소. 스타일 커스터마이징이 가능하여 프로젝트 디자인에 통합 가능
