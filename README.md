...
- **Next.js 14 (App Router)**
  - **선택 이유**: 파일 기반 라우팅을 제공하여 페이지 단위 구조를 직관적으로 관리할 수 있으며, 서버 컴포넌트·클라이언트 컴포넌트를 명확히 분리해 필요한 부분만 클라이언트 번들에 포함하도록 구성할 수 있습니다. 이를 통해 초기 렌더링 성능을 개선할 수 있고, 동적 라우팅 및 데이터 패칭 방식이 일관되어 유지보수가 용이합니다.


- **TypeScript**
  - **선택 이유**: API 응답 데이터의 타입을 명확히 정의하여 런타임 에러를 사전에 방지하고, 자동완성과 정적 타입 분석을 통해 개발 효율성과 안정성을 향상시킵니다. 함수 시그니처 변경 시 영향을 받는 부분을 컴파일 단계에서 즉시 파악할 수 있어 유지보수성과 확장성이 높습니다.

### 스타일링

- **Tailwind CSS**
  - **선택 이유**: 유틸리티 클래스 기반의 스타일링 방식으로 컴포넌트 단위 스타일을 빠르게 구성할 수 있으며, 별도의 CSS 파일 관리 없이 UI 일관성을 유지할 수 있습니다. 반응형 유틸리티가 제공되어 브레이크포인트별 레이아웃 관리가 용이합니다.

### 주요 라이브러리

- **react-day-picker**
  - **선택 이유**: 날짜 범위 선택 기능을 검증된 컴포넌트 형태로 제공하여 직접 구현 시 발생할 수 있는 오류나 엣지 케이스를 줄일 수 있습니다. 컴포넌트 구조가 단순해 Tailwind 기반 스타일 조정이 용이하고, 결제 기간 필터링에 필요한 날짜 범위 선택 기능을 쉽게 구현할 수 있습니다.
